%% BlockCoPRAM
function [x,err_hist,p,x_init] =  BlockCoPRAM(y_abs,A,J,K,max_iter,tol1,tol2,z)
%%updated 5/31/2017
...
MShat = zeros(K*J); %truncated correlation matrix 
AShat = zeros(m,K*J); %truncated sensing matrix 
...
%block marginals
for i=1:Nb
    BMarg(i,1) = norm(Marg((i-1)*J+1:i*J));%Nb x 1
end

[BMg BMgs] = sort(BMarg,'descend');
S0 = BMgs(1:K); %pick top K block-marginals
Sbhat = sort(S0); %sorted block indices
Shat = zeros(1,K*J); %sorted signal indices
for i=1:K
   Shat(1,(i-1)*J+1:i*J) = (Sbhat(i)-1)*J+1:J*Sbhat(i); %1 x J
end
%supp(Shat) = 1; figure; plot(supp); %support indicator
AShat = A(:,Shat); %M x K*J %sensing sub-matrix
...
for i = 1:card_Marg
    ii = Io(i);
    MShat = MShat + (y_abs2(ii))*AShat(ii,:)'*AShat(ii,:); % (K*J x K*J)
end
...
for t=1:max_iter
    ...
    p = sign(A*x);
    x = jsmp(p.*y_abs/sqrt(m), A/sqrt(m),J, K,10,x); %Its = 10 
