%% TreeCoPRAM
function [x,err_hist,p,x_init] =  TreeCoPRAM(y_abs,A,s,max_iter,tol1,tol2,z)
%%updated 5/31/2017
...
p = zeros(m,1); %phase matrix
error_hist(1,1) = 1;
error_hist(1,2) = 1;
Marg = zeros(1,n); %tree marginals
...
%% trees
numAtoms = 2;
d = 4; % tree order; 4 for 2D images
recflag = 'tree_exact_mex_ludwig';
Keff = numAtoms*s;

%% Truncated measurements
TAF = 'on'; %consider only truncated measurements m' < m 
TAF = 'off'; %consider all measurements = m
...
% tree marginals
supp = treeexact_smalltable_wvtree(Marg(:).^2,d,s);
Shat = find(supp==1); %sorted block indices
% figure; plot(supp); %support indicator 
...
%% decent
Afn = @(z_in) A*z_in/sqrt(m);
Atfn = @(z_out) A'*z_out/sqrt(m);

fprintf('\n#iter\t|y-Ax|\t\t|x-z|\n')
for t=1:max_iter

    p = sign(A*x);
    x = approxtreecosamp(recflag, p.*y_abs/sqrt(m), s, d, Afn, Atfn, 1e-3, 100, 0, 0);
